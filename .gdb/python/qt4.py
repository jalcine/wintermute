...
class QStringPrinter:
  ...
  def to_string(self):
    ...
    dataAsCharPointer = self.val['d'][
      'data'].cast(gdb.lookup_type("char").pointer())
...
class QListPrinter:
  ...
  class _iterator:
    ...
    def next(self):
        ...
        isLarge = self.nodetype.sizeof > gdb.lookup_type(
            'void').pointer().sizeof
        ...
        if isLarge or isStatic:  # see QList::Node::t()
            node = array.cast(
                gdb.lookup_type('QList<%s>::Node' % self.nodetype).pointer())
        else:
            node = array.cast(
                gdb.lookup_type('QList<%s>::Node' % self.nodetype))
        self.count = self.count + 1
  ...
  class _iterator:
    ...
    def concrete(self, data_node):
        node_type = gdb.lookup_type(
            'QMapNode<%s, %s>' % (self.ktype, self.vtype)).pointer()
...
class QHashPrinter:
  ...
  class _iterator:
    ...
    def __init__(self, val):
        ...
        self.ktype = self.val.type.template_argument(0)
        self.vtype = self.val.type.template_argument(1)
        self.end_node = self.d.cast(
            gdb.lookup_type('QHashData::Node').pointer())
    ...
    def firstNode(self):
        ...
        while n:
            ...
            if bucket != e:
                # print "QHashData::firstNode() in while, return *bucket
                # %s" % bucket
                return bucket
            bucketNum += 1
            bucket = self.d['buckets'][bucketNum]
            # print "QHashData::firstNode() in while, new bucket %s" %
            # bucket
            n -= 1
...
class QDatePrinter:
  ...
  def to_string(self):
    ...
    if julianDay >= 2299161:
        ...
        # Algorithm from Frequently Asked Questions about Calendars by
        # Claus Toendering
        julianDay += 32082
...
class QDateTimePrinter:
  ...
  def to_string(self):
    ...
    #increment for QAtomicInt ref; and QDate date;
    time += gdb.lookup_type(
        'int').sizeof + gdb.lookup_type('QDate').sizeof
...
class QUrlPrinter:
  ...
  def to_string(self):
    try:
        ...
        # problem with this is that if QUrlPrivate members get changed,
        # this fails
        offset = gdb.lookup_type('int').sizeof
        offset += offset % gdb.lookup_type(
            'void').pointer().sizeof  # alignment
        offset += gdb.lookup_type('QString').sizeof * 6
        offset += gdb.lookup_type('QByteArray').sizeof
        encodedOriginal = self.val['d'].cast(
            gdb.lookup_type('char').pointer())
        encodedOriginal += offset
        encodedOriginal = encodedOriginal.cast(
            gdb.lookup_type('QByteArray').pointer()).dereference()
...
class QSetPrinter:
  ...
  class _iterator:
    ...
    def next(self):
        ...
        item = node['key']
        self.hashIterator.data_node = self.hashIterator.nextNode(
            self.hashIterator.data_node)
...
def build_dictionary():
  ...
  pretty_printers_dict[re.compile('^QStringList$')] = lambda val: QListPrinter(
    val, 'QStringList', 'QString')
  pretty_printers_dict[
