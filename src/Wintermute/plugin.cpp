...
{
  const QMetaClassInfo versionField = metaObject()->classInfo (
      metaObject()->indexOfClassInfo ( "Version" ) );
  const QString value = versionField.value();
...
{
  const QMetaClassInfo versionField = metaObject()->classInfo (
      metaObject()->indexOfClassInfo ( "System Version" ) );
  const QString value = versionField.value();
  Q_ASSERT ( !value.isNull() );
  return Version::fromString ( value );
}
...
{
  Q_D ( Plugin );
  winfo ( this, "Syncing plugin settings..." );
  d->settings->sync();
}
