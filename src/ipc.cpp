...
IPCPrivate::IPCPrivate (IPC* p_qPtr) : q_ptr (p_qPtr), connection (0), adaptor (0)
{
  ...
  if (connection == 0){
    qDebug() << "Failed to connect to D-Bus.";
    Core::exit(WINTER_EXITCODE_DBUS_FAILED_TO_CONNECT);
  }
}
...
void IPC::start()
{
    ...
    const bool isStartingCoreModule = (module() == WINTER_COMMAND_LINE_IPC_CORE);
    ...
    serviceRegistered = bus->registerService (WINTER_DBUS_CONNECTION);

    /// *** Attempt to register this application into the current D-Bus session.

    if (!serviceRegistered && isStartingCoreModule) {
        /// *** @TODO: Attempt to issue a signal to the real core about this failure.
        qDebug() << "(core) Fatal: Cannot run more than one Wintermute service (" << WINTER_DBUS_CONNECTION << ")"
                 << "under the same user on the same computer";
        Core::exit (WINTER_EXITCODE_DBUS_GENERIC | WINTER_EXITCODE_NORMAL);
    }

    if (serviceRegistered){
        qDebug() << "(core) [D-Bus] Service" << WINTER_DBUS_CONNECTION << "running.";
...
void IPC::handleExit()
{
    if (module () != "master" && Core::arguments ().value ("help") == "ignore") {
        ...
        QDBusMessage reply = IPC::callMethod(WINTER_DBUS_CONNECTION, WINTER_DBUS_MODULE_MASTER, WINTER_DBUS_CONNECTION "." WINTER_DBUS_MODULE_MASTER, "quit" ,QVariantList());
